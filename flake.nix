{
  ...
  outputs = { self, nixpkgs, nixos-hardware }:
    let
      ...
      # Load the overrides/definitions generated by pip2nix
      # Assuming python-packages.nix returns an overlay function (final: prev: { ... })
      # If it returns just an attrset { pkg = ...; }, adjust accordingly.
      pythonOverlay = pkgs.callPackage ./python-packages.nix {
         # Pass dependencies needed by python-packages.nix if any, e.g.:
         inherit pythonPackages;
       };

      # Create the final python package set, applying the overlay
      finalPythonPackages = pythonPackages.extend pythonOverlay;

      # Define Python dependencies *as a list* using the final package set
      pythonDeps = with finalPythonPackages; [
        # Use package names exactly as they are available in the final set
        # (Check python-packages.nix if pip2nix renamed anything)
        adafruit-blinka
        ...
        adafruit-pureio
        libgpiod # Check name used by pip2nix/python-packages.nix if overridden
        pillow
        psutil
        pyftdi
        pyserial
        python-periphery
        pyusb
        raspberrypilib # Check name used by pip2nix/python-packages.nix if overridden
        spidev
        sysv-ipc
        # Include any packages *only* defined in python-packages.nix
      ];
